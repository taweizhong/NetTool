...
func GetAddressSetting() fyne.CanvasObject {
	...
	addressLabel := widget.NewLabel("服务器地址")
	...
	EntryBox := container.New(layout.NewGridWrapLayout(fyne.NewSize(60, 32)), portEntry)

	listenBut := widget.NewButton("监听", func() {
		PortText = portEntry.Text
		go handle.TcpServer(AddressText+":"+PortText, C)
	})

	stopBut := widget.NewButton("停止", func() {
		go handle.TcpServerClose(C)
		log.Println("stop")
	})
